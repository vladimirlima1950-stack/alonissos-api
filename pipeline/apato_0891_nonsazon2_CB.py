# apato_0891_nonsazon2_CB.py
# Equivalente 100% ao MySQL sp8_A_nonsazon2_CB



# apato_0891_nonsazon2_CB - versão multi-cliente
# Equivalente 100% ao MySQL sp8_A_nonsazon2_CB
...
def apato_0891_nonsazon2_CB(pasta_cliente):
    ...
    try:
        # Consulta equivalente ao filtro C/B
        query = """
            SELECT 
                t2.sku,
                t2.x_ordem,
            ...
            WHERE t2.class_ABC_pedidos = 'C'
              AND t2.class_ABC_valores = 'B'
        """

        df = con.execute(query).df()

        if df.empty:
            print("Nenhum SKU encontrado para classe C/B.")
            return
        ...
        con.execute("DROP TABLE IF EXISTS tb_A_trend3_somas_e_medias_fase1_CB")
        con.register("df_final_CB", res)
        con.execute("""
            CREATE TABLE tb_A_trend3_somas_e_medias_fase1_CB AS
            SELECT * FROM df_final_CB
        """)

        print("apato_0891_nonsazon2_CB executado com sucesso — versão Pandas/Numpy equivalente ao MySQL.")
...
if __name__ == "__main__":
    ...
    apato_0891_nonsazon2_CB(pasta_cliente)
