...
def run(pasta_cliente):
    ...
    try:
        # Consulta equivalente ao bloco + filtros A/B
        query = """
            SELECT 
                t2.sku,
                t2.x_ordem,
        ...
        if df.empty:
            print("Nenhum SKU encontrado para classe A/B.")
            return
        ...
        con.register("df_final_AB", res)
        con.execute("""
            CREATE TABLE tb_A_trend3_somas_e_medias_fase1_AB AS
            SELECT * FROM df_final_AB
        """)

        print("apato_0891_nonsazon2_AB executado com sucesso — versão Pandas/Numpy equivalente ao MySQL.")
