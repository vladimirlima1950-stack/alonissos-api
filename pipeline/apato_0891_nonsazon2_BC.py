# apato_0891_nonsazon2_BC.py
# Equivalente 100% ao MySQL sp8_A_nonsazon2_BC




# apato_0891_nonsazon2_BC - versão multi-cliente
# Equivalente 100% ao MySQL sp8_A_nonsazon2_BC
...
def apato_0891_nonsazon2_BC(pasta_cliente):
    ...
    try:
        # Consulta equivalente ao filtro B/C
        query = """
            SELECT 
                t2.sku,
                t2.x_ordem,
            ...
            WHERE t2.class_ABC_pedidos = 'B'
              AND t2.class_ABC_valores = 'C'
        """

        df = con.execute(query).df()

        if df.empty:
            print("Nenhum SKU encontrado para classe B/C.")
            return
        ...
        con.execute("DROP TABLE IF EXISTS tb_A_trend3_somas_e_medias_fase1_BC")
        con.register("df_final_BC", res)
        con.execute("""
            CREATE TABLE tb_A_trend3_somas_e_medias_fase1_BC AS
            SELECT * FROM df_final_BC
        """)

        print("apato_0891_nonsazon2_BC executado com sucesso — versão Pandas/Numpy equivalente ao MySQL.")
...
if __name__ == "__main__":
    ...
    apato_0891_nonsazon2_BC(pasta_cliente)
