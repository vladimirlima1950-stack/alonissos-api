...
# apato_0891_nonsazon2_AC - versão multi-cliente
...
def apato_0891_nonsazon2_AC(pasta_cliente):
    ...
    try:
        # Consulta equivalente ao filtro A/C
        query = """
            SELECT 
                t2.sku,
                t2.x_ordem,
        ...
        if df.empty:
            print("Nenhum SKU encontrado para classe A/C.")
            return
        ...
        con.execute("DROP TABLE IF EXISTS tb_A_trend3_somas_e_medias_fase1_AC")
        con.register("df_final_AC", res)
        con.execute("""
            CREATE TABLE tb_A_trend3_somas_e_medias_fase1_AC AS
            SELECT * FROM df_final_AC
        """)

        print("apato_0891_nonsazon2_AC executado com sucesso — versão Pandas/Numpy equivalente ao MySQL.")
...
if __name__ == "__main__":
    ...
    apato_0891_nonsazon2_AC(pasta_cliente)
